...
export async function saveCompletion(token: string, postId: string, platform: string, assignee: string, completed: boolean) {
  if (token === 'demo' && !isSupabaseConfigured) {
    return { completions: completed ? [{ postId, platform, assignee, completedAt: new Date().toISOString() }] : [] };
  }
  return invoke({ action: 'complete', token, postId, platform, assignee, completed });
}
...
function demoDistribution(): Distribution {
  const asset = (id: string, label: string, number: number, color: string) => {
    ...
    return { id, filename: `2046호 ${label}${number}.svg`, sizeBytes: 1_240_000, mimeType: 'image/svg+xml', thumbUrl: url, originalUrl: url, optimizedUrl: url, position: number - 1 };
  };
  return {
    id: 'demo-publication', issueNumber: '2046호', title: '2046호 SNS 카드뉴스', publishedAt: new Date().toISOString(), expiresAt: null, completions: [],
    posts: [
      { id: 'demo-post-1', position: 0, title: "'탄소중립' 반환점 돌았지만 ··· 총배출량 감축은 과제", body: '고려대 서울캠퍼스는 탄소중립 계획을 수립하고 온실가스 감축을 추진하고 있다. 목표 달성을 위해선 감축 노력을 다각화해야 한다는 지적이 나온다.', articleUrl: 'https://www.kunews.ac.kr/news/articleView.html?idxno=51212', credits: '글 | 유지원·원예지·전정현 기자\n인포그래픽 | 이지효 기자\n카드뉴스 | 송민경 미디어부장', assets: [asset('a1','지속가능',1,'#183c64'),asset('a2','지속가능',2,'#7a0019'),asset('a3','지속가능',3,'#4d6f91')] },
      { id: 'demo-post-2', position: 1, title: '재조명 내세우는 뉴라이트 끊어내야', body: '역사를 왜곡하는 뉴라이트식 재조명은 중단돼야 한다.', articleUrl: '', credits: '사설 | 고대신문 편집국', assets: [asset('b1','뉴라이트 사설',1,'#7a0019'),asset('b2','뉴라이트 사설',2,'#b43a54')] },
    ],
  };
}
