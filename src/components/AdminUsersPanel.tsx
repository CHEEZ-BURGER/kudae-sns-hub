...
export function AdminUsersPanel({ onClose }: { onClose: () => void }) {
  ...
  return <section className="panel mb-7 overflow-hidden">
    <header className="flex items-center justify-between gap-4 border-b border-line p-5">
      <div><p className="eyebrow">계정 관리</p><h2 className="mt-1 text-xl font-bold">관리자 추가</h2><p className="mt-1 text-xs leading-5 text-muted">관리자만 새 관리자를 만들 수 있습니다.</p></div>
    ...
    <div className="grid gap-6 p-5 lg:grid-cols-[minmax(0,.9fr)_minmax(320px,1.1fr)]">
      <form onSubmit={submit} className="rounded-xl border border-line bg-canvas p-4">
        <div className="mb-4 flex items-center gap-2 text-sm font-bold"><UserPlus className="text-crimson"/>새 계정 만들기</div>
      ...
      <div>
        <div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-bold"><UsersRound className="text-crimson"/>현재 관리자</div><span className="text-xs text-muted">{admins.length}명</span></div>
