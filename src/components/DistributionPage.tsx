...
function ReporterPost({ post,index,notify }: ReporterPostProps) {
  ...
  async function copyBodyOnly(){await copyText(postBody(post),'제목을 제외한 본문을 복사했습니다.');}
  ...
  return <article className="reporter-card">
    <header className="border-b border-line p-4 sm:p-6"><div className="flex gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-crimson text-xs font-bold text-white">{index+1}</span><div className="min-w-0 flex-1"><h2 className="text-lg font-bold leading-7 sm:text-xl">{displayTitle}</h2><p className="mt-1 text-xs text-muted">이미지 {imageAssets.length}장 · 영상 {videoCount}개 · 원본 {formatBytes(totalSize)}</p></div></div></header>
    <div className="p-4 sm:p-6">
      ...
      <section className="copy-tools mt-4" aria-label="제목과 본문 복사">
        ...
        <div className="copy-tool-buttons"><button className="button secondary" onClick={copyTitle}><Clipboard/>SNS 제목 복사</button><button className="button secondary" onClick={copyKoreapasTitle}><Clipboard/>고파스 제목 복사</button><button className="button secondary" onClick={copyBodyOnly}><Clipboard/>본문만 복사</button><button className="button primary" onClick={copyBody}><Clipboard/>제목+본문 복사</button></div>
      </section>
      ...
      <section className="copy-block mt-5"><div className="flex flex-wrap items-center justify-between gap-2"><b className="section-label">게시 본문 미리보기</b><div className="flex gap-2"><button className="button tiny secondary" onClick={copyBodyOnly}><Clipboard/>본문만</button><button className="button tiny secondary" onClick={copyBody}><Clipboard/>제목+본문</button></div></div><strong className="body-copy-title">{displayTitle}</strong><p className="mt-3 whitespace-pre-wrap text-sm leading-7">{content.body}</p>{content.articleUrl&&<a className="article-link" href={content.articleUrl} target="_blank" rel="noreferrer"><ExternalLink/>{content.articleUrl}</a>}{content.credits&&<p className="mt-4 whitespace-pre-wrap border-t border-line pt-4 text-xs leading-6 text-muted">{content.credits}</p>}</section>
    </div>
  </article>;
}
