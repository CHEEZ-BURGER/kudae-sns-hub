...
import { distributionShareUrl, previewForTitle, titleFromManuscript, titleWithWeekday } from '../../shared/share-preview.mjs';
...
export function AdminStudio({ session, demoMode = false }: AdminStudioProps) {
  ...
  const [publicationTitle, setPublicationTitle] = useState(() => titleWithWeekday('2046호 카드뉴스'));
  ...
  function selectSection(postId: string, sectionId: string) {
    ...
    updatePost(postId, { sectionId, confidence: 1, category: section.category, title: section.title, body: section.body, articleUrl: section.articleUrl, credits: '' });
  }
  ...
  function addPost() {
    setPosts((current) => [...current, { id: crypto.randomUUID(), groupName: '새 게시물', sectionId: '', confidence: 0, category: '보도', title: '새 게시물', body: '', articleUrl: '', credits: '', assets: [] }]);
  }
    ...
    <div className="min-h-screen bg-canvas text-ink">
      ...
      <main className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
        ...
        <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div><p className="eyebrow">{stage === 'upload' ? '새 배포 만들기' : publicationTitle}</p><h1 className="mt-2 text-3xl font-bold md:text-4xl">{stage === 'upload' ? '파일 한 번, 배포 준비 끝.' : stage === 'done' ? (editingPublicationId ? '배포 내용을 수정했습니다.' : '배포 링크가 준비됐습니다.') : (editingPublicationId ? '배포 내용을 수정해 주세요.' : '자동 매칭을 확인해 주세요.')}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">{stage === 'upload' ? '카드뉴스 이미지·영상 폴더와 원고를 올리면 게시물별 미디어와 본문을 자동으로 연결합니다.' : stage === 'edit' ? `${posts.length}개 게시물 · 미디어 ${posts.flatMap((post)=>post.assets).length}개 · ${formatBytes(totalBytes)}` : '기자는 기존 링크에서 수정된 내용을 바로 볼 수 있습니다.'}</p></div>
          <Stepper stage={stage}/>
        </div>
  ...
  return <>
    <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_330px]">
      ...
      <aside className="panel flex flex-col overflow-hidden">
        <div className="border-b border-line p-5"><p className="text-sm font-bold">자동 분석 항목</p><p className="mt-1 text-xs leading-5 text-muted">파일을 올리면 아래 순서로 정리합니다.</p></div>
...
function DoneStage({ shareUrl, reset, editing }: { shareUrl:string; reset:()=>void; editing:boolean }) {
  ...
  return <section className="panel mx-auto max-w-2xl p-7 text-center sm:p-10"><span className="success-icon"><Check/></span><p className="eyebrow mt-5">{editing?'수정 완료':'배포 완료'}</p><h2 className="mt-2 text-2xl font-bold">{editing?'기존 배포 링크에 수정 내용이 반영됐습니다.':'기자들에게 이 링크를 보내세요.'}</h2><p className="mt-2 text-sm text-muted">{editing?'기존 링크도 계속 열립니다. 변경된 카톡 미리보기 제목은 아래 링크를 새로 복사해 보내세요.':'링크를 받은 사람은 로그인 없이 이 배포만 읽을 수 있습니다.'}</p><div className="share-box mt-7"><Link2/><input readOnly value={shareUrl}/><button onClick={copy}>{copied?'복사됨':'링크 복사'}</button></div><div className="mt-6 flex flex-wrap justify-center gap-2"><a className="button secondary" href={shareUrl}>기자 화면 열기</a><button className="button ghost" onClick={reset}>배포 목록으로</button></div></section>;
}
...
function RecentPublications({ data, onDelete, onEdit, loadingEditId }: { data:Array<Record<string,string|null>>; onDelete:(id:string)=>Promise<void>; onEdit:(id:string)=>Promise<void>; loadingEditId:string }) {
  ...
  async function remove(id:string,title:string){
    ...
    setDeleting(id);
    await onDelete(id);
    setDeleting(''); setConfirming('');
  }
  return <section className="mt-10"><div className="mb-3 flex items-center justify-between"><div><h2 className="text-sm font-bold">최근 배포</h2><p className="mt-1 text-xs text-muted">최신 3개만 보관하며 새 배포가 추가되면 가장 오래된 자료를 자동 삭제합니다.</p></div><span className="text-xs text-muted">{data.length}건</span></div><div className="panel divide-y divide-line overflow-hidden">{data.slice(0,3).map((item)=><div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between" key={item.id}><div className="min-w-0"><b className="block truncate text-sm">{item.title}</b><span className="text-xs text-muted">{item.issue_number} · {item.published_at ? new Date(item.published_at).toLocaleString('ko-KR') : '작성 중'}</span>{confirming===item.id&&<span className="mt-1 block text-xs font-bold text-crimson">한 번 더 누르면 배포와 원본이 삭제됩니다.</span>}</div><div className="flex shrink-0 flex-wrap gap-2"><button className="button tiny secondary" disabled={Boolean(loadingEditId)||deleting===item.id} onClick={()=>onEdit(item.id!)}>{loadingEditId===item.id?<LoaderCircle className="animate-spin"/>:<Pencil/>}{loadingEditId===item.id?'불러오는 중':'수정'}</button>{item.share_token&&<button className="button tiny ghost" onClick={()=>copy(item.share_token!,item.title||'')}><Clipboard/>링크 복사</button>}{confirming===item.id&&<button className="button tiny ghost" onClick={()=>setConfirming('')}>취소</button>}<button className="button tiny danger" disabled={deleting===item.id||Boolean(loadingEditId)} onClick={()=>remove(item.id!,item.title??'배포')}><Trash2/>{deleting===item.id?'삭제 중':confirming===item.id?'삭제 확정':'삭제'}</button></div></div>)}</div></section>;
}
