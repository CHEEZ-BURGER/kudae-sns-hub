  ...
  return (
    <main className="grid min-h-screen bg-canvas lg:grid-cols-2">
      <section className="login-brand-panel hidden p-14 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div><img className="login-wordmark" src={wordmarkUrl} alt="The Korea University Weekly Since 1947"/><Newspaper className="mt-10" size={34}/><h1 className="mt-5 max-w-lg text-5xl font-bold leading-[1.1]">파일 찾는 시간은 줄이고,<br/>기사에 집중하세요.</h1><p className="mt-5 max-w-md text-sm leading-7 text-white/80">카드뉴스와 원고를 한 번에 정리해 기자들이 필요한 게시물만 바로 가져가게 합니다.</p></div>
      ...
      <section className="flex items-center justify-center p-5">
        <form className="panel w-full max-w-md p-7 sm:p-9" onSubmit={submit}>
          ...
          <p className="eyebrow">관리자 로그인</p><h2 className="mt-2 text-2xl font-bold">배포 작업을 시작할까요?</h2><p className="mt-2 text-sm leading-6 text-muted">발급받은 관리자 ID와 비밀번호로 로그인해 주세요.</p>
