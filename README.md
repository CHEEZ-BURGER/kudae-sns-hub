...
- 기자의 제목/본문 개별 복사, 6개 플랫폼 맞춤 모바일 배포, 순서를 기억하는 이미지 연속 복사, 개별 복사/저장, 다중 파일 Web Share, 게시물별 ZIP 다운로드
- 페이스북·고파스·에타는 이미지 전체 묶음, 인스타는 최대 10장, X는 최대 4장씩 자동 분할, 유튜브는 영상만 별도 공유
...
3. 게시물에서 페이스북·고파스·인스타·유튜브·X·에타 중 업로드할 플랫폼 선택
4. 모바일에서는 플랫폼별 `원본 공유`를 눌러 휴대폰 공유창에서 대상 앱 선택
5. X 사진은 4장씩, 인스타 사진은 10장씩 표시되는 공유 묶음을 순서대로 전달
6. 전체 공유가 되지 않는 앱에서는 `순차 복사 시작` 또는 개별 원본 저장 사용
7. `제목 복사`와 `본문 복사`를 각각 사용하고, 필요하면 `전체 ZIP`을 fallback으로 사용
...
- Web Share 동작은 모바일 OS와 대상 SNS 앱 버전에 따라 달라질 수 있습니다. `navigator.canShare({ files })`가 거부하면 순차 복사·개별 저장·ZIP 다운로드 안내가 표시됩니다.
- 원본 공유에는 파일만 전달합니다. 제목과 본문은 대상 앱이 공유 텍스트를 무시하는 경우를 피하기 위해 별도 복사 버튼으로 제공합니다.
...
- `src/lib/workflow.test.ts`, `src/lib/storage-path.test.ts`, `src/lib/platform-share.test.ts`는 파일명 그룹화, 자연 정렬, 실제 HWP 줄바꿈 기반 원고 분할, 본문 내 크레딧, 5개 업무 예시 자동 매칭, 안전한 Storage 경로, X·인스타·유튜브의 플랫폼별 공유 묶음을 검증합니다.
