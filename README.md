...
- 원고 파일명 또는 첫머리의 `2045호 카드뉴스 (木)`를 배포 제목으로 자동 가져옵니다. 한글 요일 `(목)`도 `(木)`로 정규화합니다. 관리자의 배포 제목 입력란에서 예상 미리보기 제목을 확인할 수 있습니다.
- 배포 완료 및 최근 배포의 **링크 복사**는 `share/2045-thu.html#/d/토큰` 같은 주소를 만듭니다. 해당 HTML은 JS 실행 전부터 `og:title`과 고대신문 로고를 포함합니다. 웹앱·인증·Storage는 그대로 GitHub Pages + Supabase이며 추가 서비스/요금은 없습니다.
