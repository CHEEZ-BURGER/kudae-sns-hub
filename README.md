...
1. 링크 열기
2. `원본 준비` 후 묶음 드래그 영역을 SNS 작성창으로 끌어놓기
3. 지원되지 않는 환경에서는 `전체 공유` 또는 `전체 ZIP` 사용
4. `제목 복사`와 `본문 복사`를 각각 사용
...
- 외부 SNS 작성창이 브라우저의 다중 파일 드롭을 받아야 묶음 드래그가 동작합니다. 지원하지 않는 SNS에서는 다중 파일 Web Share 또는 ZIP 다운로드를 사용합니다.
