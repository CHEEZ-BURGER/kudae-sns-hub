...
- 본문 미리보기·복사·확장 패널은 동일한 `extension/shared/post-content.mjs`를 사용해 **기사 안내문 → 기사 URL → 크레딧** 순서로 정리합니다. 기존 배포 데이터도 다시 업로드할 필요가 없고, 원문 중간 참고 링크는 보존합니다. 확장은 최신 ZIP(v2.1.1)으로 교체 후 새로고침해야 합니다.
