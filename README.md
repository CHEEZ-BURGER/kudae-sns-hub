...
6. PC Chrome 148 이상에서는 무료 확장을 한 번 설치한 뒤 `Instagram에 바로 넣기`로 다운로드·파일 선택 없이 전달
7. 모바일에서는 확장을 사용할 수 없으므로 순차 복사·원본 저장을 그대로 사용
8. 영상은 원본 다운로드 후 해당 SNS의 업로드 화면에서 선택
...
- Instagram 바로 넣기는 데스크톱 Chrome 148 이상에서만 활성화됩니다. Instagram DOM이 바뀌어 작성 창을 자동으로 열지 못해도 오류로 즉시 종료하지 않고 사용자가 `만들기 → 게시물`을 열 때까지 기다립니다.
