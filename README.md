...
6. 영상은 원본 다운로드 후 해당 SNS의 업로드 화면에서 선택
7. 한 게시물을 마치면 `다음 글`을 눌러 이어서 작업
...
- `src/lib/workflow.test.ts`, `src/lib/storage-path.test.ts`, `src/lib/post-copy.test.ts`는 파일명 그룹화, 자연 정렬, 실제 HWP 줄바꿈 기반 원고 분할, 본문 내 크레딧, 5개 업무 예시 자동 매칭, 안전한 Storage 경로, 분류·고파스 제목과 제목 포함 본문 형식을 검증합니다.
