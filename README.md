...
- 기자의 제목/본문 개별 복사, 순서를 기억하는 고파스용 연속 이미지 복붙, 개별 복사/저장, 다중 파일 Web Share, 게시물별 ZIP 다운로드
...
3. `연속 복붙 시작` → 고파스에 `Ctrl+V` → 돌아와 `다음 이미지 복사`를 순서대로 반복
4. 모바일에서는 `전체 공유`, PC 파일 업로드가 필요할 때는 `전체 ZIP` 사용
5. `제목 복사`와 `본문 복사`를 각각 사용
