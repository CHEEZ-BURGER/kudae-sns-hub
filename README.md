...
6. PC Chrome 148 이상에서는 무료 확장의 오른쪽 배포 패널에 링크를 붙여 넣고 Facebook·고파스·Instagram·YouTube·X·에타에 원본 전달. 일반 YouTube 커뮤니티 게시물에는 이미지 최대 10장, YouTube Studio에는 영상 1개를 자동 선택
...
- 원고 파일명 또는 첫머리의 `2045호 카드뉴스 (木)`를 배포 제목으로 자동 가져옵니다. 한글 요일 `(목)`도 `(木)`로 정규화하며, 원고에 요일이 없으면 링크 생성일의 요일을 자동으로 붙입니다. 관리자의 배포 제목 입력란에서 예상 미리보기 제목을 확인할 수 있습니다.
- 배포 완료 및 최근 배포의 **링크 복사**는 항상 요일이 포함된 `share/2045-thu.html#/d/토큰` 같은 주소를 만듭니다. 해당 HTML은 JS 실행 전부터 `og:title`과 호수·요일이 자동 표기된 800×400 카카오 대표 이미지를 포함합니다. 웹앱·인증·Storage는 그대로 GitHub Pages + Supabase이며 추가 서비스/요금은 없습니다.
- 정적 호스팅 특성상 빌드 때 2000~2499호 × 8종(요일 없음 또는 7요일)의 HTML과 PNG 4,000쌍을 생성합니다. `shared/share-preview.mjs`에서 범위를 늘릴 수 있습니다. PNG는 주어진 업로드 링크 디자인을 2:1 비율로 재구성하며 Pretendard와 고대신문 로고를 사용합니다. 자유 형식 제목과 범위 밖 호수는 기본 사이트 이름의 기존 주소로 돌아갑니다. 생성 페이지와 이미지에는 실제 원고, 이미지 원본, 공유 토큰을 넣지 않습니다.
...
- 본문 미리보기·복사·확장 패널은 동일한 `extension/shared/post-content.mjs`를 사용해 **기사 안내문 → 기사 URL → 크레딧** 순서로 정리합니다. 제목을 제외한 `본문만 복사`와 `제목+본문 복사`를 함께 제공합니다. 기존 배포 데이터도 다시 업로드할 필요가 없고, 원문 중간 참고 링크는 보존합니다. 확장은 최신 ZIP(v2.2.2)으로 교체 후 새로고침해야 합니다.
...
4. 확장 아이콘을 눌러 오른쪽 배포 패널 열기. 글 이동은 패널 상단에서 하며 로고를 누르면 관리자 페이지가 열립니다.
...
사이트와 패널은 그림자 없이 테두리와 색면으로 구분하며, `prefers-reduced-motion`을 존중하는 짧은 전환만 사용합니다. 사이트 토스트·브라우저 확인창·SNS 작성창 위 진행 팝업은 사용하지 않고 모든 상태를 현재 페이지나 패널 내부에 표시합니다. 확장 업데이트 직후 SNS 탭에 예전 연결 코드가 남아 있으면 패널 전체 새로고침 안내에서 해당 SNS 탭만 다시 불러올 수 있습니다.
